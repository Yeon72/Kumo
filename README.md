...
### 👤 担当パート (My Contributions)

**1. 統合管理者(Admin)システム及び統計ロジックの実装**
- Spring Data JPAを活用し、東京・大阪の4つのテーブルに分散した求人データを統合管理するロジックを設計。
- Chart.jsを用いた採用統計ダッシュボードを構築し、管理者の業務効率を改善。

**2. 求人者向け採用管理システム(ATS)の構築**
- 応募者リストのアコーディオンUI及び、履歴書の非同期(Fetch API)照会機能を実装。
- JPA Projectionを活用し、大量の応募者データ取得時のクエリパフォーマンスを最適化。

**3. プロジェクトの「火消し役」としてのリファクタリング**
- チーム内の開発遅延を解消するため、チャット機能の基盤再構築やUIバグの修正を主導。
- 既存のレガシーコードを分析し、保守性の高いコードへのリファクタリングを実施。
